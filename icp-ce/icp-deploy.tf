...
data "template_file" "create_storage_class_sh" {
  ...
  vars {
    master_node_url = "${google_compute_instance.icp-master.network_interface.0.access_config.0.nat_ip }"
...
module "icpprovision" {
    ...
    icp-host-groups = {
        ...
        proxy = "${slice(concat(google_compute_instance.icp-proxy.*.network_interface.0.network_ip,
                                list(google_compute_instance.icp-master.network_interface.0.network_ip)),
                         ...
                         var.proxy["nodes"] > 0 ? length(google_compute_instance.icp-proxy.*.network_interface.0.network_ip) :
                                                  length(google_compute_instance.icp-proxy.*.network_interface.0.network_ip) +
                                                    length(list(google_compute_instance.icp-master.network_interface.0.network_ip)))}"
    ...
    cluster_size  = "${1 + 1 + var.worker["nodes"] + var.proxy["nodes"] + var.management["nodes"]}"
...
output "ICP Console load balancer IP (external)" {
  value = "${google_compute_instance.icp-master.network_interface.0.access_config.0.nat_ip }"
}
...
output "ICP Console URL" {
  value = "https://${google_compute_instance.icp-master.network_interface.0.access_config.0.nat_ip }:8443"
}

output "ICP Registry URL" {
  value = "${google_compute_instance.icp-master.network_interface.0.access_config.0.nat_ip }:8500"
}

output "ICP Kubernetes API URL" {
  value = "https://${google_compute_instance.icp-master.network_interface.0.access_config.0.nat_ip }:8001"
}
